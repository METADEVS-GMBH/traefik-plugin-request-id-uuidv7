...
func TestOmitRequestIdInHeaderIfNoneExist(t *testing.T) {
	cfg := CreateConfig()
	cfg.Enabled = false
	...
	cid := req.Header.Get("X-Request-ID")
	if cid != "" {
		t.Errorf("Request ID has been generated")
	}
}

func TestReplaceRequestIdInHeaderIfOneExist(t *testing.T) {
	cfg := CreateConfig()
	...
	cid := req.Header.Get("X-Request-ID")
	if cid == existingCid {
		t.Errorf("Existing Request ID has not been replaced")
	}
}
